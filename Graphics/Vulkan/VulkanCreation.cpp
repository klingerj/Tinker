...
namespace Tk
{
  namespace Graphics
  {
    static void CreateDescriptorPool()
    {
      ...
      VkResult result =
        vkCreateDescriptorPool(g_vulkanContextResources.device, &descPoolCreateInfo,
                               nullptr, &g_vulkanContextResources.descriptorPool);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create descriptor pool!",
                              Core::Utility::LogSeverity::eInfo);
        return;
      }
    }

    static void CreateImageView(VkDevice device, VkFormat format,
                                VkImageAspectFlags aspectMask, VkImage image,
                                VkImageView* imageView, uint32 arrayEles)
    {
      VkImageViewCreateInfo imageViewCreateInfo = {};
      imageViewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
      imageViewCreateInfo.image = image;
      imageViewCreateInfo.viewType =
        arrayEles > 1 ? VK_IMAGE_VIEW_TYPE_2D_ARRAY : VK_IMAGE_VIEW_TYPE_2D;
      ...
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create Vulkan image view!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
    }

    static VkShaderModule CreateShaderModule(const char* shaderCode,
                                             uint32 numShaderCodeBytes, VkDevice device)
    {
      ...
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create Vulkan shader module!",
                              Core::Utility::LogSeverity::eCritical);
    ...
    void CreateSwapChainAPIObjects(SwapChainData* swapChainData,
                                   const Tk::Platform::WindowHandles* windowHandles)
    {
      uint32 newSwapChainAPIObjectsHandle =
        g_vulkanContextResources.vulkanSwapChainDataPool.Alloc();
...
// Surface
      ...
      VkResult result = vkCreateWin32SurfaceKHR(g_vulkanContextResources.instance,
                                                &win32SurfaceCreateInfo, NULL,
                                                &vulkanSwapChainData->surface);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create Win32SurfaceKHR!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
#else
      ...
      {
        optimalExtent.width =
          CLAMP(swapChainData->windowWidth, capabilities.minImageExtent.width,
                capabilities.maxImageExtent.width);
        optimalExtent.height =
          CLAMP(swapChainData->windowHeight, capabilities.minImageExtent.height,
      ...
      if (numSwapChainImages != DESIRED_NUM_SWAP_CHAIN_IMAGES)
      {
        Core::Utility::LogMsg("Platform",
                              "Swap chain unable to support number of requested images!",
                              Core::Utility::LogSeverity::eCritical);
      }
      ...
      vkGetPhysicalDeviceSurfaceFormatsKHR(g_vulkanContextResources.physicalDevice,
                                           vulkanSwapChainData->surface,
                                           &numAvailableSurfaceFormats, nullptr);

      if (numAvailableSurfaceFormats == 0)
      {
        Core::Utility::LogMsg("Platform",
                              "Zero available Vulkan swap chain surface formats!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }

      VkSurfaceFormatKHR* availableSurfaceFormats =
        (VkSurfaceFormatKHR*)g_vulkanContextResources.DataAllocator.Alloc(
          sizeof(VkSurfaceFormatKHR) * numAvailableSurfaceFormats, 1);
      vkGetPhysicalDeviceSurfaceFormatsKHR(
        g_vulkanContextResources.physicalDevice, vulkanSwapChainData->surface,
        &numAvailableSurfaceFormats, availableSurfaceFormats);

      VkSurfaceFormatKHR chosenFormat = {};
      chosenFormat.format = VK_FORMAT_UNDEFINED;
      for (uint32 uiAvailFormat = 0; uiAvailFormat < numAvailableSurfaceFormats;
           ++uiAvailFormat)
      {
        if (availableSurfaceFormats[uiAvailFormat].format == VK_FORMAT_B8G8R8A8_SRGB
            && availableSurfaceFormats[uiAvailFormat].colorSpace
      ...
      vkGetPhysicalDeviceSurfacePresentModesKHR(g_vulkanContextResources.physicalDevice,
                                                vulkanSwapChainData->surface,
                                                &numAvailablePresentModes, nullptr);

      if (numAvailablePresentModes == 0)
      {
        Core::Utility::LogMsg("Platform",
                              "Zero available Vulkan swap chain present modes!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }

      VkPresentModeKHR* availablePresentModes =
        (VkPresentModeKHR*)g_vulkanContextResources.DataAllocator.Alloc(
          sizeof(VkPresentModeKHR) * numAvailablePresentModes, 1);
      vkGetPhysicalDeviceSurfacePresentModesKHR(
        g_vulkanContextResources.physicalDevice, vulkanSwapChainData->surface,
        &numAvailablePresentModes, availablePresentModes);

      VkPresentModeKHR chosenPresentMode = VK_PRESENT_MODE_FIFO_KHR;
      for (uint32 uiAvailPresMode = 0; uiAvailPresMode < numAvailablePresentModes;
           ++uiAvailPresMode)
      {
      ...
      g_vulkanContextResources.swapChainFormat =
        chosenFormat.format; // not stored per allocated swap chain data, all swap chains
      ...
      result = vkCreateSwapchainKHR(g_vulkanContextResources.device, &swapChainCreateInfo,
                                    nullptr, &vulkanSwapChainData->swapChain);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create Vulkan swap chain!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }

      vkGetSwapchainImagesKHR(g_vulkanContextResources.device,
                              vulkanSwapChainData->swapChain, &numSwapChainImages,
                              nullptr);
      TINKER_ASSERT(numSwapChainImages != 0);

      vulkanSwapChainData->swapChainImages =
        (VkImage*)g_vulkanContextResources.DataAllocator.Alloc(
          sizeof(VkImage) * numSwapChainImages, 1);
      vulkanSwapChainData->swapChainImageViews =
        (VkImageView*)g_vulkanContextResources.DataAllocator.Alloc(
          sizeof(VkImageView) * numSwapChainImages, 1);
      ...
      {
        CreateImageView(
          g_vulkanContextResources.device, g_vulkanContextResources.swapChainFormat,
          VK_IMAGE_ASPECT_COLOR_BIT, vulkanSwapChainData->swapChainImages[uiImageView],
          &vulkanSwapChainData->swapChainImageViews[uiImageView], 1);
      }
      ...
      for (uint32 i = 0; i < DESIRED_NUM_SWAP_CHAIN_IMAGES; ++i)
      {
        ...
        *newResourceChain = {};
        ...
        newResourceChain->resourceChain[0].image =
          vulkanSwapChainData->swapChainImages[i];
        newResourceChain->resourceChain[0].imageView =
          vulkanSwapChainData->swapChainImageViews[i];
        newResourceChain->resDesc.debugLabel = "Swap chain image resource";
        newResourceChain->resDesc.dims =
          v3ui((uint32)vulkanSwapChainData->swapChainExtent.width,
               (uint32)vulkanSwapChainData->swapChainExtent.height, 1);
      ...
      for (uint32 uiFrame = 0; uiFrame < MAX_FRAMES_IN_FLIGHT; ++uiFrame)
      {
        result = vkCreateSemaphore(
          g_vulkanContextResources.device, &semaphoreCreateInfo, nullptr,
          &vulkanSwapChainData->virtualFrameSyncData[uiFrame].GPUWorkCompleteSema);

        if (result != VK_SUCCESS)
        {
          Core::Utility::LogMsg("Platform",
                                "Failed to create Vulkan gpu work complete semaphore!",
                                Core::Utility::LogSeverity::eCritical);
        }
        ...
        if (result != VK_SUCCESS)
        {
          Core::Utility::LogMsg("Platform", "Failed to create Vulkan present semaphore!",
                                Core::Utility::LogSeverity::eCritical);
        }
      }
      ...
      for (uint32 uiFrame = 0; uiFrame < MAX_FRAMES_IN_FLIGHT; ++uiFrame)
      {
        result = vkCreateFence(g_vulkanContextResources.device, &fenceCreateInfo, nullptr,
                               &vulkanSwapChainData->virtualFrameSyncData[uiFrame].Fence);
        if (result != VK_SUCCESS)
        {
          Core::Utility::LogMsg("Platform", "Failed to create Vulkan fence!",
                                Core::Utility::LogSeverity::eCritical);
        }
      }
    }
    ...
    void DestroySwapChainAPIObjects(SwapChainData* swapChainData)
    {
      ...
      VulkanSwapChainData* vulkanSwapChainData =
        g_vulkanContextResources.vulkanSwapChainDataPool.PtrFromHandle(
          swapChainAPIObjectsHandle);

      vkDeviceWaitIdle(g_vulkanContextResources.device); // TODO: move this?

      for (uint32 i = 0; i < DESIRED_NUM_SWAP_CHAIN_IMAGES; ++i)
      {
        g_vulkanContextResources.vulkanMemResourcePool.Dealloc(
          swapChainData->swapChainResourceHandles[i].m_hRes);
      }

      // Don't have to destroy swap chain VkImages
      for (uint32 uiImg = 0; uiImg < swapChainData->numSwapChainImages; ++uiImg)
      {
        vkDestroyImageView(g_vulkanContextResources.device,
                           vulkanSwapChainData->swapChainImageViews[uiImg], nullptr);
      }

      vkDestroySwapchainKHR(g_vulkanContextResources.device,
                            vulkanSwapChainData->swapChain, nullptr);
      vkDestroySurfaceKHR(g_vulkanContextResources.instance, vulkanSwapChainData->surface,
                          nullptr);
      ...
      for (uint32 uiFrame = 0; uiFrame < MAX_FRAMES_IN_FLIGHT; ++uiFrame)
      {
        ...
        vkDestroySemaphore(
          g_vulkanContextResources.device,
          vulkanSwapChainData->virtualFrameSyncData[uiFrame].ImageAvailableSema, nullptr);
        vulkanSwapChainData->virtualFrameSyncData[uiFrame].ImageAvailableSema =
          VK_NULL_HANDLE;
        vkDestroyFence(g_vulkanContextResources.device,
                       vulkanSwapChainData->virtualFrameSyncData[uiFrame].Fence, nullptr);
    ...
    CREATE_GRAPHICS_PIPELINE(CreateGraphicsPipeline)
    {
      ...
      {
        vertexShaderModule =
          CreateShaderModule((const char*)vertexShaderCode, numVertexShaderBytes,
                             g_vulkanContextResources.device);
        ++numStages;
      }
      if (numFragmentShaderBytes > 0)
      {
        fragmentShaderModule =
          CreateShaderModule((const char*)fragmentShaderCode, numFragmentShaderBytes,
                             g_vulkanContextResources.device);
        ++numStages;
      }
      ...
      for (uint32 uiDesc = 0; uiDesc < numDescriptorLayoutHandles; ++uiDesc)
      {
        ...
        {
          descriptorSetLayouts[uiDesc] =
            g_vulkanContextResources.descLayouts[descLayoutID].layout;
        }
      }
      ...
      VkResult result =
        vkCreatePipelineLayout(g_vulkanContextResources.device, &pipelineLayoutCreateInfo,
      ...
      {
        for (uint32 blendState = 0; blendState < VulkanContextResources::eMaxBlendStates;
             ++blendState)
        {
          for (uint32 depthState = 0;
               depthState < VulkanContextResources::eMaxDepthStates; ++depthState)
          {
            VkPipeline& graphicsPipeline =
              g_vulkanContextResources.psoPermutations
                .graphicsPipeline[shaderID][blendState][depthState];

            DepthCullState depthCullState = GetVkDepthCullState(depthState);
            VkPipelineColorBlendAttachmentState colorBlendAttachment =
              GetVkBlendState(blendState);
            ...
            for (uint32 uiFmt = 0; uiFmt < Min(numColorRTs, (uint32)ARRAYCOUNT(formats));
                 ++uiFmt)
            {
      ...
      vkDestroyShaderModule(g_vulkanContextResources.device, fragmentShaderModule,
                            nullptr);
    ...
    DESTROY_GRAPHICS_PIPELINE(DestroyGraphicsPipeline)
    {
      ...
      for (uint32 uiblendState = 0;
           uiblendState < VulkanContextResources::eMaxBlendStates; ++uiblendState)
      {
        for (uint32 uiDepthState = 0;
             uiDepthState < VulkanContextResources::eMaxDepthStates; ++uiDepthState)
        {
          VkPipeline& graphicsPipeline =
            g_vulkanContextResources.psoPermutations
              .graphicsPipeline[shaderID][uiblendState][uiDepthState];
    ...
    CREATE_COMPUTE_PIPELINE(CreateComputePipeline)
    {
      ...
      {
        computeShaderModule =
          CreateShaderModule((const char*)computeShaderCode, numComputeShaderBytes,
                             g_vulkanContextResources.device);
      }
      ...
      else
      {
        ...
        result =
          vkCreateComputePipelines(g_vulkanContextResources.device, VK_NULL_HANDLE, 1,
                                   &pipelineCreateInfo, nullptr, &computePipeline);

        if (result != VK_SUCCESS)
        {
          Core::Utility::LogMsg("Platform", "Failed to create Vulkan compute pipeline!",
                                Core::Utility::LogSeverity::eCritical);
          TINKER_ASSERT(0);
        }
      }

      vkDestroyShaderModule(g_vulkanContextResources.device, computeShaderModule,
                            nullptr);
    ...
    void DestroyAllPSOPerms()
    {
      ...
      for (uint32 shaderID = 0; shaderID < VulkanContextResources::eMaxShaders;
           ++shaderID)
      {
        DestroyGraphicsPipeline(shaderID);
      }

      for (uint32 shaderID = 0; shaderID < VulkanContextResources::eMaxShadersCompute;
           ++shaderID)
      {
                                               ...
                                               const char* debugLabel)
    {
      ...
      *newResourceChain = {};
      ...
      uint32 AllocatorIndex = 0xFF'FF'FF'FF;
      if (propertyFlags == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
      {
        AllocatorIndex =
          g_vulkanContextResources.eVulkanMemoryAllocatorDeviceLocalBuffers;
      }
      else
      {
        AllocatorIndex =
          g_vulkanContextResources.eVulkanMemoryAllocatorHostVisibleBuffers;
      }
      ...
      for (uint32 uiBuf = 0; uiBuf < NumCopies; ++uiBuf)
      {
        ...
        VkResult result =
          vkCreateBuffer(g_vulkanContextResources.device, &bufferCreateInfo, nullptr,
                         &newResource->buffer);
        if (result != VK_SUCCESS)
        {
          Core::Utility::LogMsg("Platform", "Failed to create buffer!",
                                Core::Utility::LogSeverity::eCritical);
          TINKER_ASSERT(0);
        }

        VkMemoryRequirements memRequirements = {};
        vkGetBufferMemoryRequirements(g_vulkanContextResources.device,
                                      newResource->buffer, &memRequirements);

        VulkanMemAlloc newAlloc =
          g_vulkanContextResources.GPUMemAllocators[AllocatorIndex].Alloc(
            memRequirements);
        result = vkBindBufferMemory(g_vulkanContextResources.device, newResource->buffer,
                                    newAlloc.allocMem, newAlloc.allocOffset);
        if (result != VK_SUCCESS)
        {
          Core::Utility::LogMsg("Platform", "Failed to bind buffer memory!",
                                Core::Utility::LogSeverity::eCritical);
          TINKER_ASSERT(0);
        }
    ...
    static ResourceHandle CreateImageResource(uint32 imageFormat, uint32 imageUsageFlags,
                                              uint32 width, uint32 height,
                                              uint32 numArrayEles, const char* debugLabel)
    {
      uint32 newResourceHandle = g_vulkanContextResources.vulkanMemResourcePool.Alloc();
      TINKER_ASSERT(newResourceHandle != TINKER_INVALID_HANDLE);
      ...
      *newResourceChain = {};
      ...
      VkResult result = vkCreateImage(g_vulkanContextResources.device, &imageCreateInfo,
                                      nullptr, &newResource->image);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create image!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
      ...
      // Pick the correct gpu memory allocator
      // TODO: this will change once the user can create allocators via the graphics layer
      const uint32 AllocatorIndex =
        g_vulkanContextResources.eVulkanMemoryAllocatorDeviceLocalImages;
      VkMemoryRequirements memRequirements = {};
      vkGetImageMemoryRequirements(g_vulkanContextResources.device, newResource->image,
                                   &memRequirements);

      VulkanMemAlloc newAlloc =
        g_vulkanContextResources.GPUMemAllocators[AllocatorIndex].Alloc(memRequirements);
      result = vkBindImageMemory(g_vulkanContextResources.device, newResource->image,
                                 newAlloc.allocMem, newAlloc.allocOffset);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to bind image memory!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
      ...
      // Create image view
      CreateImageView(g_vulkanContextResources.device, GetVkImageFormat(imageFormat),
                      GetVkImageAspectFlags(imageFormat), newResource->image,
                      &newResource->imageView, numArrayEles);
    ...
    ResourceHandle CreateResource(const ResourceDesc& resDesc)
    {
      ...
      switch (resDesc.resourceType)
      {
        ...
        {
          newHandle =
            CreateBufferResource(resDesc.dims.x, resDesc.bufferUsage, resDesc.debugLabel);
          break;
        }

        case ResourceType::eImage2D:
        {
          newHandle = CreateImageResource(resDesc.imageFormat, resDesc.imageUsageFlags,
                                          resDesc.dims.x, resDesc.dims.y,
                                          resDesc.arrayEles, resDesc.debugLabel);
          break;
        }

        default:
        {
          Core::Utility::LogMsg("Platform", "Invalid resource description type!",
                                Core::Utility::LogSeverity::eCritical);
          return newHandle;
        }
      }
      ...
      VulkanMemResourceChain* newResourceChain =
        g_vulkanContextResources.vulkanMemResourcePool.PtrFromHandle(newHandle.m_hRes);
        ...
        switch (resourceChain->resDesc.resourceType)
        {
          ...
          case ResourceType::eImage2D:
          {
            if (resource->image != VK_NULL_HANDLE)
            {
              ...
              vkDestroyImageView(g_vulkanContextResources.device, resource->imageView,
                                 nullptr);
            }
            break;
          }
    ...
    void CreateSamplers()
    {
      ...
      VkResult result =
        vkCreateSampler(g_vulkanContextResources.device, &samplerCreateInfo, nullptr,
                        &g_vulkanContextResources.linearSampler);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create sampler!",
                              Core::Utility::LogSeverity::eCritical);
        return;
      }
    }
    ...
    DescriptorHandle CreateDescriptor(uint32 descriptorLayoutID)
    {
      ...
      VulkanDescriptorLayout& vulkanDescriptorLayout =
        g_vulkanContextResources.descLayouts[descriptorLayoutID];
      ...
      uint32 newDescriptorHandle =
        g_vulkanContextResources.vulkanDescriptorResourcePool.Alloc();
        ...
        if (descriptorSetLayout != VK_NULL_HANDLE)
        {
          ...
          VulkanDescriptor& vulkanDescriptor =
            g_vulkanContextResources.vulkanDescriptorResourcePool
              .PtrFromHandle(newDescriptorHandle)
              ->resourceChain[uiFrame];

          VkResult result =
            vkAllocateDescriptorSets(g_vulkanContextResources.device, &descSetAllocInfo,
                                     &vulkanDescriptor.descriptorSet);
          if (result != VK_SUCCESS)
          {
            Core::Utility::LogMsg("Platform", "Failed to create Vulkan descriptor set!",
                                  Core::Utility::LogSeverity::eCritical);
            TINKER_ASSERT(0);
          }
        }
    ...
    bool CreateDescriptorLayout(uint32 descriptorLayoutID,
                                const DescriptorLayout* descriptorLayout)
    {
      ...
      for (uint32 uiDesc = 0; uiDesc < MAX_BINDINGS_PER_SET; ++uiDesc)
      {
        ...
        if (type < DescriptorType::eMax)
        {
          ...
          descLayoutBinding[numBindings].descriptorCount =
            descriptorLayout->params[uiDesc].amount;
          descLayoutBinding[numBindings].binding = uiDesc;
          descLayoutBinding[numBindings].stageFlags = VK_SHADER_STAGE_VERTEX_BIT
      ...
      {
        Core::Utility::LogMsg("Platform",
                              "No descriptors passed to VulkanCreateDescriptorLayout()!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }

      VkDescriptorSetLayout* descriptorSetLayout =
        &g_vulkanContextResources.descLayouts[descriptorLayoutID].layout;
      *descriptorSetLayout = VK_NULL_HANDLE;
      ...
      VkResult result = vkCreateDescriptorSetLayout(
        g_vulkanContextResources.device, &descLayoutInfo, nullptr, descriptorSetLayout);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform",
      ...
      memcpy(&g_vulkanContextResources.descLayouts[descriptorLayoutID].bindings,
             &descriptorLayout->params[0], sizeof(DescriptorLayout));
      return true;
    }
    ...
    void DestroyAllDescLayouts()
    {
      ...
      {
        VulkanDescriptorLayout& vulkanDescriptorLayout =
          g_vulkanContextResources.descLayouts[desc];
        VkDescriptorSetLayout& descLayout = vulkanDescriptorLayout.layout;
        if (descLayout != VK_NULL_HANDLE)
        {
          vkDestroyDescriptorSetLayout(g_vulkanContextResources.device, descLayout,
                                       nullptr);
    ...
    {
      vkDestroyDescriptorPool(g_vulkanContextResources.device,
                              g_vulkanContextResources.descriptorPool, nullptr);
