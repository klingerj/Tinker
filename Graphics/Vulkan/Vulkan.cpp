...
namespace Tk
{
  namespace Graphics
  {
    ...
    // This code exists so that we can preallocate proper VkDeviceMemory's before creating
    // any buffers for faster GPU memory allocation
    static void InitGPUMemAllocators()
    {
...

      {
        ...
        VkBufferUsageFlags AllPossibleFlags = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT
                                              | VK_BUFFER_USAGE_TRANSFER_DST_BIT
                                              | VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
        VkBuffer TestBuffer = VK_NULL_HANDLE;
        VkResult result =
          CreateBuffer(0, 64u, AllPossibleFlags, VK_SHARING_MODE_EXCLUSIVE, &TestBuffer);
        ...
        vkGetBufferMemoryRequirements(g_vulkanContextResources.device, TestBuffer,
                                      &memRequirements);
        // Done with the buffer
        vkDestroyBuffer(g_vulkanContextResources.device, TestBuffer, nullptr);

        uint32 memoryTypeIndex = ChooseMemoryTypeBits(
          memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        g_vulkanContextResources
          .GPUMemAllocators[g_vulkanContextResources
                              .eVulkanMemoryAllocatorDeviceLocalBuffers]
          .Init(DEVICE_LOCAL_BUFFER_HEAP_SIZE, memoryTypeIndex,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                properties.limits.nonCoherentAtomSize);
      }

      {
        VkBufferUsageFlags AllPossibleFlags =
        ...
        g_vulkanContextResources
          ...
          .Init(HOST_VISIBLE_HEAP_SIZE, memoryTypeIndex,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                properties.limits.nonCoherentAtomSize);

        // Note: special!!! this memory block is going to be persistently mapped for now
        result =
          vkMapMemory(g_vulkanContextResources.device,
                      g_vulkanContextResources
                        .GPUMemAllocators[g_vulkanContextResources
                                            .eVulkanMemoryAllocatorHostVisibleBuffers]
                        .m_GPUMemory,
      ...
      {
        VkImageUsageFlags AllPossibleFlags = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT
                                             | VK_IMAGE_USAGE_SAMPLED_BIT
                                             | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
        VkImage TestImage = VK_NULL_HANDLE;
        VkResult result =
          CreateImage(0, VK_IMAGE_TYPE_2D, VK_FORMAT_R8G8B8A8_SRGB, { 64u, 64u, 1u }, 6u,
                      4u, VK_IMAGE_TILING_OPTIMAL, AllPossibleFlags,
                      VK_SHARING_MODE_EXCLUSIVE, &TestImage);
        if (result != VK_SUCCESS)
        {
        ...
        vkGetImageMemoryRequirements(g_vulkanContextResources.device, TestImage,
                                     &memRequirements);
        // Done with the image
        ...
        g_vulkanContextResources
          ...
          .Init(DEVICE_LOCAL_IMAGE_HEAP_SIZE, memoryTypeIndex,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                properties.limits.nonCoherentAtomSize);
      }
    }
    ...
    int InitVulkan(const Tk::Platform::WindowHandles* platformWindowHandles)
    {
      ...
      g_vulkanContextResources.vulkanDescriptorResourcePool.Init(VULKAN_RESOURCE_POOL_MAX,
                                                                 16);
      g_vulkanContextResources.vulkanSwapChainDataPool.Init(
        NUM_SWAP_CHAINS_STARTING_ALLOC_SIZE, 16);
      ...
      for (uint32 sid = 0; sid < VulkanContextResources::eMaxShaders; ++sid)
      {
        ...
        for (uint32 bs = 0; bs < VulkanContextResources::eMaxBlendStates; ++bs)
        {
          ...
          {
            g_vulkanContextResources.psoPermutations.graphicsPipeline[sid][bs][ds] =
              VK_NULL_HANDLE;
          }
        }
      }
...
// TODO: different platform surface extension
      ...
      {
        Core::Utility::LogMsg("Platform", enabledExtensionNames[uiReqExt],
                              Core::Utility::LogSeverity::eInfo);
      }
      ...
      VkExtensionProperties* availableExtensions =
        (VkExtensionProperties*)g_vulkanContextResources.DataAllocator.Alloc(
          sizeof(VkExtensionProperties) * numAvailableExtensions, 1);
      vkEnumerateInstanceExtensionProperties(nullptr, &numAvailableExtensions,
                                             availableExtensions);
      //Core::Utility::LogMsg("Platform", "******** Available Instance Extensions:
      //********", Core::Utility::LogSeverity::eInfo); for (uint32 uiAvailExt = 0;
      //uiAvailExt < numAvailableExtensions; ++uiAvailExt)
      //{
      //Core::Utility::LogMsg("Platform", availableExtensions[uiAvailExt].extensionName,
      //Core::Utility::LogSeverity::eInfo);
      //}
      ...
      Core::Utility::LogMsg("Platform", "******** Requested Instance Layers: ********",
                            Core::Utility::LogSeverity::eInfo);
      for (uint32 uiReqLayer = 0; uiReqLayer < numRequestedLayers; ++uiReqLayer)
      {
        Core::Utility::LogMsg("Platform", requestedLayers[uiReqLayer],
                              Core::Utility::LogSeverity::eInfo);
      }
      ...
      {
        Core::Utility::LogMsg("Platform", "Zero available instance layers!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }

      VkLayerProperties* availableLayers =
        (VkLayerProperties*)g_vulkanContextResources.DataAllocator.Alloc(
          sizeof(VkLayerProperties) * numAvailableLayers, 1);
      vkEnumerateInstanceLayerProperties(&numAvailableLayers, availableLayers);

      if (0)
      {
        Core::Utility::LogMsg("Platform", "******** Available Instance Layers: ********",
                              Core::Utility::LogSeverity::eInfo);
        for (uint32 uiAvailLayer = 0; uiAvailLayer < numAvailableLayers; ++uiAvailLayer)
        {
          Core::Utility::LogMsg("Platform", availableLayers[uiAvailLayer].layerName,
                                Core::Utility::LogSeverity::eInfo);
        }
      }
      ...
      for (uint32 uiReqLayer = 0; uiReqLayer < numRequestedLayers; ++uiReqLayer)
      {
        isRequestedLayerSupported.PushBackRaw(
          false); // starts not supported, then check all available layers

        for (uint32 uiAvailLayer = 0; uiAvailLayer < numAvailableLayers; ++uiAvailLayer)
        {
          if (!strcmp(availableLayers[uiAvailLayer].layerName,
                      requestedLayers[uiReqLayer]))
          {
      ...
      for (uint32 uiReqLayer = 0; uiReqLayer < numRequestedLayers; ++uiReqLayer)
      {
        ...
        {
          Core::Utility::LogMsg("Platform", "Requested instance layer not supported!",
                                Core::Utility::LogSeverity::eCritical);
          Core::Utility::LogMsg("Platform", requestedLayers[uiReqLayer],
                                Core::Utility::LogSeverity::eCritical);
          TINKER_ASSERT(0);
      ...
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create Vulkan instance!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
      ...
      dbgUtilsMsgCreateInfo.sType =
        VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
      dbgUtilsMsgCreateInfo.messageSeverity =
        VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT
        | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT
        | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
      dbgUtilsMsgCreateInfo.messageType =
      ...
      PFN_vkCreateDebugUtilsMessengerEXT dbgCreateFunc =
        (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
          g_vulkanContextResources.instance, "vkCreateDebugUtilsMessengerEXT");
      if (dbgCreateFunc)
      {
        dbgCreateFunc(g_vulkanContextResources.instance, &dbgUtilsMsgCreateInfo, nullptr,
                      &g_vulkanContextResources.debugMessenger);
      }
      else
      {
        Core::Utility::LogMsg("Platform",
                              "Failed to get create debug utils messenger proc addr!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
#endif
      ...
      vkEnumeratePhysicalDevices(g_vulkanContextResources.instance, &numPhysicalDevices,
                                 nullptr);

      if (numPhysicalDevices == 0)
      {
        Core::Utility::LogMsg("Platform", "Zero available physical devices!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }

      VkPhysicalDevice* physicalDevices =
        (VkPhysicalDevice*)g_vulkanContextResources.DataAllocator.Alloc(
          sizeof(VkPhysicalDevice) * numPhysicalDevices, 1);
      vkEnumeratePhysicalDevices(g_vulkanContextResources.instance, &numPhysicalDevices,
                                 physicalDevices);

      const uint32 numRequiredPhysicalDeviceExtensions = 1;
      const char*
        requiredPhysicalDeviceExtensions[numRequiredPhysicalDeviceExtensions] = {
          VK_KHR_SWAPCHAIN_EXTENSION_NAME
        };
      Core::Utility::LogMsg("Platform", "******** Requested Device Extensions: ********",
                            Core::Utility::LogSeverity::eInfo);
      for (uint32 uiReqExt = 0; uiReqExt < numRequiredPhysicalDeviceExtensions;
           ++uiReqExt)
      {
        Core::Utility::LogMsg("Platform", requiredPhysicalDeviceExtensions[uiReqExt],
                              Core::Utility::LogSeverity::eInfo);
      }
      ...
      for (uint32 uiPhysicalDevice = 0; uiPhysicalDevice < numPhysicalDevices;
           ++uiPhysicalDevice)
      {
        ...
        physicalDeviceVulkan12Features.sType =
          VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES;
        physicalDeviceVulkan13Features = {};
        physicalDeviceVulkan13Features.sType =
        ...
        if (physicalDeviceProperties.limits.maxPushConstantsSize
        ...
        if (physicalDeviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
        {
          ...
          vkGetPhysicalDeviceQueueFamilyProperties(currPhysicalDevice, &numQueueFamilies,
                                                   nullptr);

          VkQueueFamilyProperties* queueFamilyProperties =
            (VkQueueFamilyProperties*)g_vulkanContextResources.DataAllocator.Alloc(
              sizeof(VkQueueFamilyProperties) * numQueueFamilies, 1);
          vkGetPhysicalDeviceQueueFamilyProperties(currPhysicalDevice, &numQueueFamilies,
          ...
          for (uint32 uiQueueFamily = 0; uiQueueFamily < numQueueFamilies;
               ++uiQueueFamily)
          {
          ...
          vkEnumerateDeviceExtensionProperties(
            currPhysicalDevice, nullptr, &numAvailablePhysicalDeviceExtensions, nullptr);

          if (numAvailablePhysicalDeviceExtensions == 0)
          {
            Core::Utility::LogMsg("Platform", "Zero available device extensions!",
                                  Core::Utility::LogSeverity::eCritical);
            TINKER_ASSERT(0);
          }

          VkExtensionProperties* availablePhysicalDeviceExtensions =
            (VkExtensionProperties*)g_vulkanContextResources.DataAllocator.Alloc(
              sizeof(VkExtensionProperties) * numAvailablePhysicalDeviceExtensions, 1);
          vkEnumerateDeviceExtensionProperties(currPhysicalDevice, nullptr,
                                               &numAvailablePhysicalDeviceExtensions,
                                               availablePhysicalDeviceExtensions);

          //Core::Utility::LogMsg("Platform", "******** Available Device Extensions:
          //********", Core::Utility::LogSeverity::eInfo); for (uint32 uiAvailExt = 0;
          //uiAvailExt < numAvailablePhysicalDeviceExtensions; ++uiAvailExt)
          //{
          //Core::Utility::LogMsg("Platform",
          //availablePhysicalDeviceExtensions[uiAvailExt].extensionName,
          //Core::Utility::LogSeverity::eInfo);
          //}
          ...
          for (uint32 uiReqExt = 0; uiReqExt < numRequiredPhysicalDeviceExtensions;
               ++uiReqExt)
          {
            for (uint32 uiAvailExt = 0; uiAvailExt < numAvailablePhysicalDeviceExtensions;
                 ++uiAvailExt)
            {
              if (!strcmp(availablePhysicalDeviceExtensions[uiAvailExt].extensionName,
                          requiredPhysicalDeviceExtensions[uiReqExt]))
              {
               ...
               ++uiReqExt)
          {
            ...
            {
              Core::Utility::LogMsg("Platform",
                                    "Requested device extension not supported!",
                                    Core::Utility::LogSeverity::eCritical);
              Core::Utility::LogMsg("Platform",
                                    requiredPhysicalDeviceExtensions[uiReqExt],
                                    Core::Utility::LogSeverity::eCritical);
      ...
      {
        Core::Utility::LogMsg("Platform", "No physical device chosen!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
      ...
      {
        Core::Utility::LogMsg("Graphics", "Timestamps not supported on this device",
                              Core::Utility::LogSeverity::eInfo);
      }
      else
      {
        // supported, need to query the period since it is not the same across vendors
        g_vulkanContextResources.timestampPeriod =
      ...
      if (0)
      {
        ...
        vkGetPhysicalDeviceMemoryProperties(g_vulkanContextResources.physicalDevice,
                                            &memoryProperties);
        {
          Core::Utility::LogMsg("Platform", "******** Device Memory Properties: ********",
                                Core::Utility::LogSeverity::eInfo);

          for (uint32 uiMemType = 0; uiMemType < memoryProperties.memoryTypeCount;
               ++uiMemType)
          {
            Core::Utility::LogMsg("Platform", "****************",
                                  Core::Utility::LogSeverity::eInfo);
            ...
            VkMemoryPropertyFlags propertyFlags =
              memoryProperties.memoryTypes[uiMemType].propertyFlags;

            Core::Utility::LogMsg("Platform",
                                  "Heap Properties:", Core::Utility::LogSeverity::eInfo);
            if (propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
            {
              Core::Utility::LogMsg("Platform", "- Device local",
                                    Core::Utility::LogSeverity::eInfo);
            }
            if (propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
            {
              Core::Utility::LogMsg("Platform", "- Host visible",
                                    Core::Utility::LogSeverity::eInfo);
            }
            if (propertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
            {
              Core::Utility::LogMsg("Platform", "- Host coherent",
                                    Core::Utility::LogSeverity::eInfo);
            }
            if (propertyFlags & VK_MEMORY_PROPERTY_HOST_CACHED_BIT)
            {
              Core::Utility::LogMsg("Platform", "- Host cached",
                                    Core::Utility::LogSeverity::eInfo);
            }
            if (propertyFlags & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT)
            {
              Core::Utility::LogMsg("Platform", "- Lazily allocated",
                                    Core::Utility::LogSeverity::eInfo);
            }
            ...
            Core::Utility::LogMsg("Platform",
                                  "Heap Size:", Core::Utility::LogSeverity::eInfo);
      ...
      deviceQueueCreateInfos[0].queueFamilyIndex =
      ...
      result = vkCreateDevice(g_vulkanContextResources.physicalDevice, &deviceCreateInfo,
                              nullptr, &g_vulkanContextResources.device);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create Vulkan device!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
...
// Debug utils labels / debug markers
      ...
      {
        Core::Utility::LogMsg("Platform",
                              "Failed to get create debug utils marker proc addr!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
#endif
      ...
      vkGetDeviceQueue(g_vulkanContextResources.device,
                       g_vulkanContextResources.graphicsQueueIndex, 0,
      ...
      result =
        vkCreateCommandPool(g_vulkanContextResources.device, &commandPoolCreateInfo,
                            nullptr, &g_vulkanContextResources.commandPool);
      if (result != VK_SUCCESS)
      {
        Core::Utility::LogMsg("Platform", "Failed to create Vulkan command pool!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
      ...
      if (timestampsAvailable)
      {
        ...
        result = vkCreateQueryPool(g_vulkanContextResources.device, &queryPoolCreateInfo,
                                   NULL, &g_vulkanContextResources.queryPoolTimestamp);
        if (result != VK_SUCCESS)
        {
          Core::Utility::LogMsg("Graphics", "Failed to timestamp query pool!",
                                Core::Utility::LogSeverity::eCritical);
        }
      }
    ...
    void DestroyVulkan()
    {
      ...
      vkDestroyQueryPool(g_vulkanContextResources.device,
                         g_vulkanContextResources.queryPoolTimestamp, nullptr);

      vkDestroyCommandPool(g_vulkanContextResources.device,
                           g_vulkanContextResources.commandPool, nullptr);

      DestroyAllPSOPerms();
      DestroyAllDescLayouts();

      vkDestroySampler(g_vulkanContextResources.device,
                       g_vulkanContextResources.linearSampler, nullptr);

      for (uint32 uiAlloc = 0;
           uiAlloc < ARRAYCOUNT(g_vulkanContextResources.GPUMemAllocators); ++uiAlloc)
      {
      ...
      PFN_vkDestroyDebugUtilsMessengerEXT dbgDestroyFunc =
        (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
          g_vulkanContextResources.instance, "vkDestroyDebugUtilsMessengerEXT");
      if (dbgDestroyFunc)
      {
        dbgDestroyFunc(g_vulkanContextResources.instance,
                       g_vulkanContextResources.debugMessenger, nullptr);
      }
      else
      {
        Core::Utility::LogMsg("Platform",
                              "Failed to get destroy debug utils messenger proc addr!",
                              Core::Utility::LogSeverity::eCritical);
        TINKER_ASSERT(0);
      }
#endif
