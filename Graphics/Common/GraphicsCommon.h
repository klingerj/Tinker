...
#define THREADGROUP_ROUND(x, tg) ((x + tg - 1) / tg)
    ...
    // Try to free a descriptor set with a resource handle, which can happen if all
    // handles are just plain uint32.
    ...
    struct DescriptorHandle
    {
      ...
      {
        return m_hDesc == other.m_hDesc && m_layoutID == other.m_layoutID;
      }
...
#define DefaultDescHandle_Invalid DescriptorHandle()
    ...
    struct GraphicsCommandStream
    {
      ...
      void Clear()
      {
        ...
        //shouldnt' be needed for correctness but could be helpful for debug
      }
      ...
      void CmdDraw(uint32 numIndices, uint32 numInstances, uint32 vertexOffset,
                   uint32 indexOffset, uint32 shaderID, uint32 blendState,
                   uint32 depthState, ResourceHandle indexBufferHandle,
                   uint32 numDescriptors, DescriptorHandle* descriptors,
                   const char* dbgLabel = "Draw")
      {
        ...
        memcpy(command->m_descriptors, descriptors,
               sizeof(DescriptorHandle) * numDescriptors);
      }

      void CmdDispatch(uint32 threadGroupsX, uint32 threadGroupsY, uint32 threadGroupsZ,
                       uint32 shaderID, uint32 numDescriptors,
                       DescriptorHandle* descriptors, const char* dbgLabel = "Dispatch")
      {
        TINKER_ASSERT(numDescriptors <= MAX_DESCRIPTOR_SETS_PER_SHADER);
      ...
      void CmdSetScissor(int32 scissorOffsetX, int32 scissorOffsetY, uint32 scissorWidth,
                         uint32 scissorHeight, const char* dbgLabel = "SetScissor")
      {
        GraphicsCommand* command = GetNextCommand();
      ...
      void CmdSetViewport(float viewportOffsetX, float viewportOffsetY,
                          float viewportWidth, float viewportHeight,
                          float viewportMinDepth, float viewportMaxDepth,
                          const char* dbgLabel = "SetViewport")
      {
        GraphicsCommand* command = GetNextCommand();
      ...
      void CmdCommandBufferBegin(CommandBuffer cmdBuf,
                                 const char* dbgLabel = "CmdBufferBegin")
      {
        GraphicsCommand* command = GetNextCommand();
      ...
      void CmdCommandBufferEnd(CommandBuffer cmdBuf,
                               const char* dbgLabel = "CmdBufferEnd")
      {
        GraphicsCommand* command = GetNextCommand();
      ...
      void CmdRenderPassBegin(uint32 renderWidth, uint32 renderHeight, uint32 numColorRTs,
                              ResourceHandle* colorRTs, ResourceHandle depthRT,
                              const char* dbgLabel = "RenderPassBegin")
      {
      ...
      void CmdLayoutTransition(ResourceHandle imageHandle, uint32 startLayout,
                               uint32 endLayout, const char* dbgLabel = "Transition")
      {
        GraphicsCommand* command = GetNextCommand();
      ...
      void CmdClear(ResourceHandle imageHandle, const v4f& clearValue,
                    const char* dbgLabel = "Clear")
      {
        GraphicsCommand* command = GetNextCommand();
      ...
      void CmdCopy(ResourceHandle srcBufferHandle, ResourceHandle dstBufferHandle,
                   uint32 sizeInBytes, const char* dbgLabel = "Copy")
      {
        GraphicsCommand* command = GetNextCommand();
...
#define WRITE_DESCRIPTOR_SIMPLE(name)                                                    \
  void name(DescriptorHandle descSetHandle,                                              \
            const DescriptorSetDataHandles* descSetDataHandles)
    WRITE_DESCRIPTOR_SIMPLE(WriteDescriptorSimple);

#define WRITE_DESCRIPTOR_ARRAY(name)                                                     \
  void name(DescriptorHandle descSetHandle, uint32 numEntries, ResourceHandle* entries,  \
            uint32 updateFlags)
    WRITE_DESCRIPTOR_ARRAY(WriteDescriptorArray);

#define SUBMIT_CMDS_IMMEDIATE(name)                                                      \
  void name(Tk::Graphics::GraphicsCommandStream* graphicsCommandStream,                  \
            Tk::Graphics::CommandBuffer commandBuffer)
...
#define CREATE_DESCRIPTOR_LAYOUT(name)                                                   \
  bool name(uint32 descLayoutID, const DescriptorLayout* descLayout)
    CREATE_DESCRIPTOR_LAYOUT(CreateDescriptorLayout);

#define CREATE_GRAPHICS_PIPELINE(name)                                                   \
  bool name(void* vertexShaderCode, uint32 numVertexShaderBytes,                         \
            void* fragmentShaderCode, uint32 numFragmentShaderBytes, uint32 shaderID,    \
...
#define CREATE_COMPUTE_PIPELINE(name)                                                    \
  bool name(void* computeShaderCode, uint32 numComputeShaderBytes, uint32 shaderID,      \
            uint32* descriptorLayoutHandles, uint32 numDescriptorLayoutHandles)
...
#define NUM_SWAP_CHAINS_STARTING_ALLOC_SIZE 16
    ...
    void ProcessGraphicsCommandStream(
      const Tk::Graphics::GraphicsCommandStream* graphicsCommandStream);
