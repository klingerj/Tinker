...
namespace Tk
{
  namespace Graphics
  {
    namespace ShaderManager
    {
      ...
      static bool LoadShader(const char* vertexShaderFileName,
                             const char* fragmentShaderFileName, uint32 shaderID,
                             ...
                             uint32* descLayouts, uint32 numDescLayouts)
      {
        ...
        if (vertexShaderFileName)
        {
          ...
          Tk::Platform::ReadEntireFile(vertexShaderFileName, vertexShaderFileSize,
                                       vertexShaderBuffer);
        }

        if (fragmentShaderFileName)
        {
          fragmentShaderFileSize =
            Tk::Platform::GetEntireFileSize(fragmentShaderFileName);
          fragmentShaderBuffer =
            g_ShaderBytecodeAllocator.Alloc(fragmentShaderFileSize, 1);
          TINKER_ASSERT(fragmentShaderBuffer);
          Tk::Platform::ReadEntireFile(fragmentShaderFileName, fragmentShaderFileSize,
                                       fragmentShaderBuffer);
        }

        const bool created = Tk::Graphics::CreateGraphicsPipeline(
          vertexShaderBuffer, vertexShaderFileSize, fragmentShaderBuffer,
          fragmentShaderFileSize, shaderID, pipelineFormats.numColorRTs,
          pipelineFormats.colorRTFormats, pipelineFormats.depthFormat, descLayouts,
          numDescLayouts);
        return created;
      }

      static bool LoadComputeShader(const char* computeShaderFileName, uint32 shaderID,
                                    uint32* descLayouts, uint32 numDescLayouts)
      {
        ...
        if (computeShaderFileName)
        {
          ...
          Tk::Platform::ReadEntireFile(computeShaderFileName, computeShaderFileSize,
                                       computeShaderBuffer);
        }

        const bool created =
          Tk::Graphics::CreateComputePipeline(computeShaderBuffer, computeShaderFileSize,
      ...
      void LoadAllShaders()
      {
        ...
        const char* shaderFilePaths[numShaderFilepaths] = {
          SHADERS_SPV_PATH "blit_VS.spv",       SHADERS_SPV_PATH "tonemapping_PS.spv",
          SHADERS_SPV_PATH "basic_VS.spv",      SHADERS_SPV_PATH "basic_PS.spv",
          SHADERS_SPV_PATH "animpoly_VS.spv",   SHADERS_SPV_PATH "animpoly_PS.spv",
          SHADERS_SPV_PATH "imgui_VS.spv",      SHADERS_SPV_PATH "imgui_PS.spv",
          SHADERS_SPV_PATH "grayscale_CS.spv",  SHADERS_SPV_PATH "blit_clear_VS.spv",
          SHADERS_SPV_PATH "blit_clear_PS.spv", SHADERS_SPV_PATH "blit_PS.spv",
        };
        ...
        bOk = LoadShader(shaderFilePaths[0], shaderFilePaths[11],
                         Graphics::SHADER_ID_QUAD_BLIT_TONEMAP, pipelineFormats,
                         descLayouts, 2);
        TINKER_ASSERT(bOk);
        ...
        bOk = LoadShader(shaderFilePaths[0], shaderFilePaths[11],
                         Graphics::SHADER_ID_QUAD_BLIT_RGBA8, pipelineFormats,
                         descLayouts, 2);
        TINKER_ASSERT(bOk);
        ...
        pipelineFormats.numColorRTs = 1;
        pipelineFormats.colorRTFormats[0] = ImageFormat::TheSwapChainFormat;
        bOk = LoadShader(shaderFilePaths[9], shaderFilePaths[10],
                         Graphics::SHADER_ID_QUAD_BLIT_CLEAR, pipelineFormats,
                         descLayouts, 1);
        TINKER_ASSERT(bOk);
        ...
        pipelineFormats.colorRTFormats[0] = ImageFormat::RGBA16_Float;
        bOk =
          LoadShader(shaderFilePaths[6], shaderFilePaths[7],
                     Graphics::SHADER_ID_IMGUI_DEBUGUI, pipelineFormats, descLayouts, 2);
        TINKER_ASSERT(bOk);

        // TODO: temporary, need to resolve how to handle RT formats
        pipelineFormats.colorRTFormats[0] = ImageFormat::TheSwapChainFormat;
        bOk = LoadShader(shaderFilePaths[6], shaderFilePaths[7],
                         Graphics::SHADER_ID_IMGUI_DEBUGUI_RGBA8, pipelineFormats,
                         descLayouts, 2);
        TINKER_ASSERT(bOk);
        ...
        bOk = LoadShader(shaderFilePaths[2], nullptr, Graphics::SHADER_ID_BASIC_ZPrepass,
                         pipelineFormats, descLayouts, 2);
        TINKER_ASSERT(bOk);
        ...
        pipelineFormats.depthFormat = ImageFormat::Depth_32F;
        bOk =
          LoadShader(shaderFilePaths[2], shaderFilePaths[3],
                     Graphics::SHADER_ID_BASIC_MainView, pipelineFormats, descLayouts, 3);
        TINKER_ASSERT(bOk);
        ...
        pipelineFormats.colorRTFormats[0] = ImageFormat::RGBA16_Float;
        pipelineFormats.depthFormat = ImageFormat::Depth_32F;
        bOk = LoadShader(shaderFilePaths[4], shaderFilePaths[5],
                         Graphics::SHADER_ID_ANIMATEDPOLY_MainView, pipelineFormats,
                         descLayouts, 2);
        TINKER_ASSERT(bOk);
        ...
        bOk = LoadComputeShader(shaderFilePaths[8], Graphics::SHADER_ID_COMPUTE_COPY,
                                descLayouts, 4);
        TINKER_ASSERT(bOk);
      }
      ...
      void LoadAllShaderResources()
      {
        ...
        bOk = Tk::Graphics::CreateDescriptorLayout(
          Graphics::DESCLAYOUT_ID_BINDLESS_CONSTANTS, &descriptorLayout);
        TINKER_ASSERT(bOk);
        ...
        bOk = Tk::Graphics::CreateDescriptorLayout(
          Graphics::DESCLAYOUT_ID_BINDLESS_TEXTURES_RGBA8_SAMPLED, &descriptorLayout);
        TINKER_ASSERT(bOk);

        descriptorLayout.InitInvalid();
        descriptorLayout.params[0].type =
          Tk::Graphics::DescriptorType::eArrayOfTexturesRW;
        descriptorLayout.params[0].amount = DESCRIPTOR_BINDLESS_ARRAY_LIMIT;
        bOk = Tk::Graphics::CreateDescriptorLayout(
          Graphics::DESCLAYOUT_ID_BINDLESS_TEXTURES_RGBA8_RW, &descriptorLayout);
        TINKER_ASSERT(bOk);

        descriptorLayout.InitInvalid();
        descriptorLayout.params[0].type = Tk::Graphics::DescriptorType::eSampledImage;
        descriptorLayout.params[0].amount = 1;
        bOk = Tk::Graphics::CreateDescriptorLayout(Graphics::DESCLAYOUT_ID_QUAD_BLIT_TEX,
                                                   &descriptorLayout);
        TINKER_ASSERT(bOk);
        ...
        bOk = Tk::Graphics::CreateDescriptorLayout(Graphics::DESCLAYOUT_ID_QUAD_BLIT_VBS,
                                                   &descriptorLayout);
        TINKER_ASSERT(bOk);
        ...
        descriptorLayout.params[2].type = Tk::Graphics::DescriptorType::eSSBO;
        descriptorLayout.params[2].amount = 1;
        bOk = Tk::Graphics::CreateDescriptorLayout(Graphics::DESCLAYOUT_ID_IMGUI_VBS,
                                                   &descriptorLayout);
        TINKER_ASSERT(bOk);
        ...
        descriptorLayout.params[0].type = Tk::Graphics::DescriptorType::eSampledImage;
        descriptorLayout.params[0].amount = 1;
        bOk = Tk::Graphics::CreateDescriptorLayout(Graphics::DESCLAYOUT_ID_IMGUI_TEX,
                                                   &descriptorLayout);
        TINKER_ASSERT(bOk);
        ...
        descriptorLayout.params[2].type = Tk::Graphics::DescriptorType::eSSBO;
        descriptorLayout.params[2].amount = 1;
        bOk = Tk::Graphics::CreateDescriptorLayout(Graphics::DESCLAYOUT_ID_ASSET_VBS,
                                                   &descriptorLayout);
        TINKER_ASSERT(bOk);
        ...
        descriptorLayout.params[0].type = Tk::Graphics::DescriptorType::eSSBO;
        descriptorLayout.params[0].amount = 1;
        bOk = Tk::Graphics::CreateDescriptorLayout(Graphics::DESCLAYOUT_ID_POSONLY_VBS,
                                                   &descriptorLayout);
        TINKER_ASSERT(bOk);
