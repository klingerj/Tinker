...
namespace Tk
{
  namespace Graphics
  {
    ...
    static_assert(
      ARRAYCOUNT(MultiBufferedStatusFromBufferUsage)
      == BufferUsage::eMax); // Don't forget to add one here if enum is added to
    ...
    void CreateSwapChain(const Tk::Platform::WindowHandles* windowHandles, uint32 width,
                         uint32 height)
    {
      uint32 dataIndex =
        SwapChainDataMap.Insert(windowHandles->hWindow, {}); // TODO: make this api better
    ...
    void CreateContext(const Tk::Platform::WindowHandles* windowHandles)
    {
      ...
      if (result)
      {
        Core::Utility::LogMsg("Platform", "Failed to init graphics backend!",
                              Core::Utility::LogSeverity::eCritical);
      }

      SwapChainDataMap.Reserve(
        NUM_SWAP_CHAINS_STARTING_ALLOC_SIZE); // TODO the hash map data structure needs
                                              // resize still so...
    }
...
//TODO: just call CreateContext()?
    ...
    void WindowResize(const Tk::Platform::WindowHandles* windowHandles,
                      uint32 newWindowWidth, uint32 newWindowHeight)
    {
    ...
    ResourceHandle GetCurrentSwapChainImage(
    ...
    {
      TINKER_ASSERT(graphicsCommandStream->m_numCommands
                    <= graphicsCommandStream->m_maxCommands);
      ...
      else
      {
        ...
        {
          TINKER_ASSERT(graphicsCommandStream->m_graphicsCommands[uiCmd].m_commandType
                        < GraphicsCommand::eMax);
          const GraphicsCommand& currentCmd =
            graphicsCommandStream->m_graphicsCommands[uiCmd];

          switch (currentCmd.m_commandType)
          {
            case GraphicsCommand::eDrawCall:
            {
              const bool psoChange = currentShaderID != currentCmd.m_shader
              ...
              if (psoChange)
              {
                RecordCommandBindShader(currentCmdBuf, currentShaderID, currentBlendState,
                                        currentDepthState);
              }
              ...
              for (uint32 uiDesc = 0; uiDesc < MAX_DESCRIPTOR_SETS_PER_SHADER; ++uiDesc)
              {
                if (currDescriptors[uiDesc] != currentCmd.m_descriptors[uiDesc])
                {
                  ...
                  if (currentCmd.m_descriptors[uiDesc]
                      != Graphics::DefaultDescHandle_Invalid)
                  {
                    RecordCommandBindDescriptor(currentCmdBuf, currentShaderID,
                                                BindPoint::eGraphics,
                                                currentCmd.m_descriptors[uiDesc], uiDesc);
                  }
                }
              }

              RecordCommandDrawCall(currentCmdBuf, currentCmd.m_indexBufferHandle,
                                    currentCmd.m_numIndices, currentCmd.m_numInstances,
                                    currentCmd.m_vertexOffset, currentCmd.m_indexOffset,
            ...
            case GraphicsCommand::eDispatch:
            {
              ...
              for (uint32 uiDesc = 0; uiDesc < MAX_DESCRIPTOR_SETS_PER_SHADER; ++uiDesc)
              {
                if (currDescriptors[uiDesc] != currentCmd.m_descriptors[uiDesc])
                {
                  ...
                  if (currentCmd.m_descriptors[uiDesc]
                      != Graphics::DefaultDescHandle_Invalid)
                  {
                    RecordCommandBindDescriptor(currentCmdBuf, currentCmd.m_shader,
                                                BindPoint::eCompute,
                                                currentCmd.m_descriptors[uiDesc], uiDesc);
                  }
                }
              ...
              RecordCommandDispatch(currentCmdBuf, currentCmd.m_threadGroupsX,
                                    currentCmd.m_threadGroupsY,
                                    currentCmd.m_threadGroupsZ, currentCmd.debugLabel);
              break;
            }

            case GraphicsCommand::eCopy:
            {
              RecordCommandMemoryTransfer(
                currentCmdBuf, currentCmd.m_sizeInBytes, currentCmd.m_srcBufferHandle,
                currentCmd.m_dstBufferHandle, currentCmd.debugLabel);
              break;
            }

            case GraphicsCommand::ePushConstant:
            {
              RecordCommandPushConstant(currentCmdBuf, &currentCmd.m_pushConstantData[0],
                                        ARRAYCOUNT(currentCmd.m_pushConstantData)
                                          * sizeof(uint8),
            ...
            {
              RecordCommandSetViewport(
                currentCmdBuf, currentCmd.m_viewportOffsetX, currentCmd.m_viewportOffsetY,
                currentCmd.m_viewportWidth, currentCmd.m_viewportHeight,
                currentCmd.m_viewportMinDepth, currentCmd.m_viewportMaxDepth);
              break;
            }

            case GraphicsCommand::eSetScissor:
            {
              RecordCommandSetScissor(
                currentCmdBuf, currentCmd.m_scissorOffsetX, currentCmd.m_scissorOffsetY,
                currentCmd.m_scissorWidth, currentCmd.m_scissorHeight);
              break;
            }
            ...
            {
              RecordCommandRenderPassBegin(
                currentCmdBuf, currentCmd.m_numColorRTs, &currentCmd.m_colorRTs[0],
                currentCmd.m_depthRT, currentCmd.m_renderWidth, currentCmd.m_renderHeight,
                currentCmd.debugLabel);
              break;
            }
            ...
            {
              RecordCommandTransitionLayout(
                currentCmdBuf, currentCmd.m_imageHandle, currentCmd.m_startLayout,
                currentCmd.m_endLayout, currentCmd.debugLabel);
              break;
            }
    ...
    void SubmitFrameToGPU(const Tk::Platform::WindowHandles* windowHandles,
    ...
    void CreateAllDefaultResources(
      Tk::Graphics::GraphicsCommandStream* graphicsCommandStream)
    {
...

      {
        ...
        desc.imageUsageFlags =
          Tk::Graphics::ImageUsageFlags::UAV | Tk::Graphics::ImageUsageFlags::TransferDst;
        desc.arrayEles = 1;
        tex.res = CreateResource(desc);
        tex.clearValue = v4f(0.0, 0.0, 0.0, 0.0);
      ...
      graphicsCommandStream->CmdCommandBufferBegin(
        cmdBuf, "Begin default texture initialization cmd buffer");

      for (uint32 i = DefaultResourceID::eTextureIndexStart; i < DefaultResourceID::eMax;
           ++i)
      {
        DefaultResource& currTex = DefaultResources[i];
        graphicsCommandStream->CmdLayoutTransition(
          currTex.res, Graphics::ImageLayout::eUndefined,
          Graphics::ImageLayout::eTransferDst,
          "Transition default texture to clear optimal");
        graphicsCommandStream->CmdClear(currTex.res, currTex.clearValue,
                                        "Clear default texture");
        graphicsCommandStream->CmdLayoutTransition(
          currTex.res, Graphics::ImageLayout::eTransferDst, currTex.defaultState,
          "Transition default texture to shader read");
      }
