...
#define ENTRY_POINT_NAME_WCHAR L"main"
    ...
    static void AppendArgs_(const wchar_t** argsToAppend, uint32 numArgsToAppend,
                            Tk::Core::Vector<const wchar_t*>& argsOut)
    {
...
#define AppendArgsToList(args, argsList) AppendArgs_(args, ARRAYCOUNT(args), argsList);
    ...
    static uint32 CompileFile(CComPtr<IDxcCompiler3> pCompiler, CComPtr<IDxcUtils> pUtils,
                              CComPtr<IDxcIncludeHandler> pIncludeHandler,
                              const wchar_t* args, uint32 numArgs,
                              const wchar_t* shaderFilepath,
                              const wchar_t* shaderFilenameWithExt)
    {
      ...
      HRESULT compileStatus = pCompiler->Compile(
        &Source, (LPCWSTR*)args, numArgs, pIncludeHandler, IID_PPV_ARGS(&pResults));
      ...
      if (SUCCEEDED(pResults->GetOutput(DXC_OUT_OBJECT, IID_PPV_ARGS(&pShader), nullptr))
          && pShader != nullptr)
      {
        ...
        uint32 shaderFilenameNoExtLen =
          (uint32)wcslen(shaderFilenameWithExt) - hlslExtLen;
        ...
        uint32 fileErr = Tk::Platform::WriteEntireFile(
          shaderFilepathSpv.m_data, (uint32)pShader->GetBufferSize(),
          (uint8*)pShader->GetBufferPointer());
        if (!fileErr)
        {
    ...
    uint32 CompileAllShadersVK()
    {
      ...
      mbstowcs_s(&numCharsWritten, currShaderFilepath, ARRAYCOUNT(currShaderFilepath),
                 SHADERS_SRC_DIR, strlen(SHADERS_SRC_DIR));
      ...
      for (uint32 uiShaderType = 0; uiShaderType < ShaderType::Max; ++uiShaderType)
      {
        ...
        Tk::Platform::FileHandle findFileHandle = Tk::Platform::FindFileOpen(
          ShaderFileSuffixRegexs[uiShaderType], shaderFilenameStart, numCharsRemaining);
        uint32 findFileError = findFileHandle.h == findFileHandle.eInvalidValue;
        while (!findFileError)
        {
          printf("\nCompiling: %ls...\n", shaderFilenameStart);

          errorCode = CompileFile(g_pCompiler, g_pUtils, g_pIncludeHandler,
                                  (const wchar_t*)g_args.Data(), g_args.Size(),
                                  currShaderFilepath, shaderFilenameStart);
          if (errorCode == ErrCode::Success || errorCode == ErrCode::HasWarnings)
          {
            // TODO: add entry to hashmap for material library, get the bytecode from the
            // compile call
          }
          else
          {
          ...
          findFileError = Tk::Platform::FindFileNext(findFileHandle, shaderFilenameStart,
                                                     numCharsRemaining);
        }
