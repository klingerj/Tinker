...
static GAME_UPDATE(GameUpdateStub)
{
  return 0;
}
...
static bool ReloadGameCode(Win32GameCode* GameCode)
{
  ...
  HANDLE gameDllFileHandle =
    CreateFile(GAME_DLL_PATH, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
  ...
  if (gameDllFileHandle == INVALID_HANDLE_VALUE)
  {
    ...
    if (errorCode == 32)
    {
      // game dll currently being used by another process - not a big deal, probably being
      // written to during build. Don't log an error
    }
    else
    {
      // some other error, log a failure
      Tk::Core::Utility::LogMsg("Platform", "Failed to get handle to game dll to reload!",
  ...
  {
    Tk::Core::Utility::LogMsg("Platform", "Loading game dll!",
                              Tk::Core::Utility::LogSeverity::eInfo);
    Tk::Core::Utility::LogMsg("Platform", GAME_DLL_PATH,
                              Tk::Core::Utility::LogSeverity::eInfo);
    ...
    {
      GameCode->GameUpdate =
        (Tk::Platform::game_update*)GetProcAddress(GameCode->GameDll, "GameUpdate");
      GameCode->GameDestroy =
        (Tk::Platform::game_destroy*)GetProcAddress(GameCode->GameDll, "GameDestroy");
      GameCode->GameWindowResize = (Tk::Platform::game_window_resize*)GetProcAddress(
        GameCode->GameDll, "GameWindowResize");
...
namespace Tk
{
  namespace Platform
  {
    ...
    ENQUEUE_WORKER_THREAD_JOB_LIST(EnqueueWorkerThreadJobList_Assisted)
    {
      ...
      uint32 NumMainThreadJobs =
        NumJobsPerThread; // main thread never does any leftover jobs for now
      ThreadPool::EnqueueJobSubList(JobList, NumJobs - NumMainThreadJobs);

      // Main thread work
      for (uint32 uiJob = NumJobs - NumMainThreadJobs; uiJob < JobList->m_numJobs;
           ++uiJob)
      {
        (*(JobList->m_jobs[uiJob]))();
        JobList->m_jobs[uiJob]->m_done = 1;
    ...
    INIT_NETWORK_CONNECTION(InitNetworkConnection)
    {
      ...
      {
        Tk::Core::Utility::LogMsg("Platform", "Failed to init network client!",
                                  Tk::Core::Utility::LogSeverity::eCritical);
        return 1;
      }
      else
      {
        Tk::Core::Utility::LogMsg("Platform", "Successfully initialized network client.",
                                  Tk::Core::Utility::LogSeverity::eInfo);
        return 0;
      }
    }
    ...
    END_NETWORK_CONNECTION(EndNetworkConnection)
    {
      ...
      {
        Tk::Core::Utility::LogMsg("Platform", "Failed to cleanup network client!",
                                  Tk::Core::Utility::LogSeverity::eCritical);
        return 1;
      }
      else
      {
        Tk::Core::Utility::LogMsg("Platform", "Successfully cleaned up network client.",
                                  Tk::Core::Utility::LogSeverity::eInfo);
        return 0;
      }
    }
    ...
    SEND_MESSAGE_TO_SERVER(SendMessageToServer)
    {
      ...
      {
        Tk::Core::Utility::LogMsg("Platform", "Failed to send message to server!",
                                  Tk::Core::Utility::LogSeverity::eCritical);
        return 1;
      }
      else
...
{
  POINT screenCenter = { (LONG)g_GlobalAppParams.m_windowWidth / 2,
                         (LONG)g_GlobalAppParams.m_windowHeight / 2 };
  ...
  switch (uMsg)
  {
    ...
    case WM_SIZE:
    {
      if (wParam == 1) // window minimized - can't create swap chain with size 0
      {
        ...
        g_windowResized =
          true; // swap chain will be recreated later when this flag is checked
      }
      else
      {
    ...
    case WM_ACTIVATEAPP:
    {
      ...
      {
        Tk::Core::Utility::LogMsg(
          "Platform", "Failed to change process priority when changing window focus!",
          Tk::Core::Utility::LogSeverity::eCritical);
      }
      else
      {
        Tk::Core::Utility::LogMsg("Platform", "Changing process priority!",
                                  Tk::Core::Utility::LogSeverity::eInfo);
        if (wParam)
        {
          Tk::Core::Utility::LogMsg("Platform", "ABOVE_NORMAL",
                                    Tk::Core::Utility::LogSeverity::eInfo);
        }
        else
        {
          Tk::Core::Utility::LogMsg("Platform", "NORMAL",
                                    Tk::Core::Utility::LogSeverity::eInfo);
        }
      }
      ...
      {
        LockCursor((HWND)g_WindowHandles.hWindow);
        SetCursorPos((int)g_GlobalAppParams.m_windowWidth / 2,
                     (int)g_GlobalAppParams.m_windowHeight / 2);
      }
                    ...
                    int nCmdShow)
{
...

  {
    ...
    {
      Tk::Core::Utility::LogMsg("Platform", "Failed to register window class!",
                                Tk::Core::Utility::LogSeverity::eCritical);
      return 1;
    }

    RECT windowDims = { 0, 0, (LONG)g_GlobalAppParams.m_windowWidth,
                        (LONG)g_GlobalAppParams.m_windowHeight };
    AdjustWindowRect(&windowDims, WS_OVERLAPPEDWINDOW | WS_VISIBLE, FALSE);

    HWND windowHandle = CreateWindowEx(
      0, windowClass.lpszClassName, "Tinker", WS_OVERLAPPEDWINDOW | WS_VISIBLE,
      CW_USEDEFAULT, CW_USEDEFAULT, windowDims.right - windowDims.left,
      windowDims.bottom - windowDims.top, 0, 0, hInstance, 0);

    if (!windowHandle)
    {
      Tk::Core::Utility::LogMsg("Platform", "Failed to create window!",
                                Tk::Core::Utility::LogSeverity::eCritical);
      return 1;
    }
...
// NOTE: we need all dll's loaded before calling SymInitialize in order for stack tracing
// to work in every dll/module
#ifdef ENABLE_MEM_TRACKING
    if (!SymInitialize(GetCurrentProcess(), NULL, TRUE))
    {
      Tk::Core::Utility::LogMsg("Platform",
                                "Unable to initialize debug sym lib for stack tracing!",
  ...
  {
    {
...

      {
        ...
        if (g_windowResized)
        {
          g_GameCode.GameWindowResize(&g_WindowHandles, g_GlobalAppParams.m_windowWidth,
...

      {
        ...
        int error =
          g_GameCode.GameUpdate(g_GlobalAppParams.m_windowWidth,
                                g_GlobalAppParams.m_windowHeight, &g_inputStateDeltas);
        if (error != 0)
        {
          Tk::Core::Utility::LogMsg(
            "Platform", "Error occurred in game code! Shutting down application.",
            Tk::Core::Utility::LogSeverity::eCritical);
          runGame = false;
          break;
        }
