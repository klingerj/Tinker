...
namespace Tk
{
  ...
  namespace Platform
  {
    ...
    {
      // TODO: tighten this up for other platforms, probably roll it into a window
      // creation function
      uint64 hInstance;
      uint64 hWindow;
    };
...
#define READ_ENTIRE_FILE(name)                                                           \
  TINKER_API uint32 name(const char* filename, uint32 fileSizeInBytes, uint8* buffer)
...
#define MAKE_DIRECTORY(name) TINKER_API void name(const char* pathname)
    MAKE_DIRECTORY(MakeDirectory);

#define FILE_EXISTS(name) TINKER_API bool name(const char* filename)
    FILE_EXISTS(CheckFileExists);

#define FIND_FILE_OPEN(name)                                                             \
  TINKER_API FileHandle name(const char* dirWithFileExts, wchar_t* outFilename,          \
                             uint32 outFilenameMax)
    FIND_FILE_OPEN(FindFileOpen);

#define FIND_FILE_NEXT(name)                                                             \
  TINKER_API uint32 name(FileHandle prevFindFileHandle, wchar_t* outFilename,            \
                         uint32 outFilenameMax)
...
#define WALK_STACK_TRACE(name)                                                           \
  uint32 name(StackTraceEntry** topOfStack,                                              \
              Tk::Core::LinearAllocator& stackEntryAllocator)
...
#define IMGUI_CREATE(name)                                                               \
  TINKER_API void name(ImGuiContext* context, ImGuiMemAllocFuncWrapper mallocWrapper,    \
                       ImGuiMemFreeFuncWrapper freeWrapper)
...
#define IMGUI_DESTROY(name) TINKER_API void name()
    ...
    typedef struct mousecode_state
    {
      union
      {
        ...
        // - maybe refactor that into a more general button state, we will probably use it
        // for game pad input too
      };
    } MousecodeState;
...
#define GAME_UPDATE(name)                                                                \
  uint32 name(uint32 windowWidth, uint32 windowHeight,                                   \
              const Tk::Platform::InputStateDeltas* inputStateDeltas)
    typedef GAME_UPDATE(game_update);

#define GAME_DESTROY(name) void name()
    typedef GAME_DESTROY(game_destroy);

#define GAME_WINDOW_RESIZE(name)                                                         \
  void name(const Tk::Platform::WindowHandles* windowHandles, uint32 newWindowWidth,     \
            uint32 newWindowHeight)
