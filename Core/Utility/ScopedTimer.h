    ...
    namespace Utility
    {
      ...
      private:
        char m_msg[MAX_MSG_LEN
                   + TIMER_MSG_SUFFIX_LEN] = {}; // TODO: use better string system here
      ...
      public:
        ...
        ScopedTimer(const char (&msg)[strLen])
          : m_msgSizeBeforeTimer(0)
        {
          const uint32 numCharsToCopy =
            strLen - 1; // don't copy null byte, assumes string literals
          static_assert(numCharsToCopy + TIMER_MSG_SUFFIX_LEN + MAX_TIME_DIGITS
                        < MAX_MSG_LEN); // make sure final appended message will fit
        ...
        ~ScopedTimer()
        {
          ...
          _ultoa_s((uint32)duration.count(), m_msg + m_msgSizeBeforeTimer,
                   MAX_TIME_DIGITS, 10);
