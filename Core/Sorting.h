...
#define CMP_LT_FUNC(name) bool name(const void* A, const void* B)
    ...
    void Merge(uint8* data, uint32 numElesLeft, uint32 numElesRight, uint32 eleSize,
               CmpFunc Compare, const uint8* tmpList)
    {
      ...
      while (1)
      {
        ...
        // Copy over left's next ele if i is not done counting AND either j is done, or
        // ith < jth ele
        if ((j >= numElesRight
             || Compare(&tmpList[i * eleSize], &tmpList[(numElesLeft + j) * eleSize]))
            && i < numElesLeft)
        {
    ...
    template <typename CmpFunc>
    void MergeSortRecursive(uint8* data, uint32 numEles, uint32 eleSize, CmpFunc Compare,
                            uint8* tmpList)
    {
      if (numEles == 1)
      {
      ...
      MergeSortRecursive(tmpList + midpoint * eleSize, rightSize, eleSize, Compare,
                         right);
