...
namespace Tk
{
  namespace Core
  {
    namespace Asset
    {
      // This implementation of OBJ loading is only set up to properly load well-formed
      // OBJ files exported from Autodesk Maya. It only loads positions, normals, UVs, and
      // indices. It also assumes the mesh has been triangulated already! It is generally
      // free to crash or misbehave when loading a nonconforming OBJ file.
      ...
      TINKER_API void ParseOBJ(Tk::Core::LinearAllocator& PosAllocator,
                               Tk::Core::LinearAllocator& UVAllocator,
                               Tk::Core::LinearAllocator& NormalAllocator,
                               Tk::Core::LinearAllocator& IndexAllocator,
                               OBJParseScratchBuffers& ScratchBuffers,
                               const uint8* EntireFileBuffer, uint64 FileSize,
                               uint32* OutVertCount);
...
#pragma pack(pop)
      ...
      TINKER_API void SaveBMP(Buffer* outputBuffer, uint8* inputData, uint32 width,
                              uint32 height, uint16 bitsPerPx);
    } //namespace Asset
  } //namespace Core
} //namespace Tk
