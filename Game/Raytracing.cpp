...
void RayMeshIntersectNaive(const Tk::Core::Raytracing::Ray& ray, v3f* triData,
                           uint32 numTris, Tk::Core::Raytracing::Intersection& isx)
{
...
void RaytraceTest()
{
  ...
  for (uint32 px = 0; px < width; ++px)
  {
    for (uint32 py = 0; py < height; ++py)
    {
      ...
      {
        v3f* meshNormals =
          (v3f*)data.m_vertexBufferData_Normal; // skip pos and uvs in buffer
        ...
        // BGRA
        img[py * width + px] = (uint32)(channel[2]
                                        | (channel[1] << 8)
                                        | (channel[0] << 16)
                                        | (channel[3] << 24));
      }
  ...
  uint32 fileErr =
    Tk::Platform::WriteEntireFile("..\\Output\\TestImages\\raytraceOutput.bmp",
                                  (uint32)imgBuffer.m_sizeInBytes, imgBuffer.m_data);
  if (fileErr)
  {
    Tk::Core::Utility::LogMsg("Platform", "Failed to write raytracing test output image!",
