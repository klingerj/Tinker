...
void CreateDefaultGeometry(Tk::Graphics::GraphicsCommandStream* graphicsCommandStream)
{
  ...
  // Default Quad
  {
    ...
    Graphics::MemoryMappedBufferPtr stagingBufferPtrPos =
      Graphics::MapResource(stagingBufferPos);
    ...
    Graphics::MemoryMappedBufferPtr stagingBufferPtrUV =
      Graphics::MapResource(stagingBufferUV);
    ...
    Graphics::MemoryMappedBufferPtr stagingBufferPtrNorm =
      Graphics::MapResource(stagingBufferNorm);
    ...
    Graphics::MemoryMappedBufferPtr stagingBufferPtrIdx =
      Graphics::MapResource(stagingBufferIdx);
...
{
  // Map
  Graphics::MemoryMappedBufferPtr indexBuf =
    Graphics::MapResource(prim->indexBufferHandle);
  Graphics::MemoryMappedBufferPtr vertexBuf =
    Graphics::MapResource(prim->vertexBufferHandle);
  ...
  for (uint32 vtx = 0; vtx < prim->numVertices; ++vtx)
  {
    ...
    else
    {
      ...
      ((v4f*)vertexBuf.m_ptr)[vtx] =
        v4f(cosf(amt) * scale, sinf(amt) * scale, 0.0f, 1.0f);
    }
  }
...
void DrawAnimatedPoly(TransientPrim* prim, uint32 shaderID, uint32 blendState,
                      uint32 depthState,
                      Graphics::GraphicsCommandStream* graphicsCommandStream)
{
  ...
  graphicsCommandStream->CmdDraw((prim->numVertices - 1) * 3, 1, 0, 0, shaderID,
                                 blendState, depthState, prim->indexBufferHandle,
