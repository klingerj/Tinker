...
static const char* demoMeshAssetNames[numDemoMeshAssets] = {
  "UnitSphere", "UnitCube", "ChickenLowPoly", "SheepLowPoly", "WolfLowPoly",
};
...
static const char* demoTextureFilePaths[numDemoTextureAssets] = {
  ASSETS_PATH "checkerboard512.bmp", ASSETS_PATH "checkerboardRGB512.bmp"
};
...
namespace AssetCooker
{
  ...
  void GetCookedDataFileName(const char* inAssetFileName, char* outCookedFileName,
                             uint32 outNameMaxLen)
  {
    ...
    const Tk::Core::Hash nameHash =
      HASH_64_RUNTIME(inAssetFileName, (uint32)strlen(inAssetFileName));
  ...
  bool IsCookedDataFilePresentForAsset(const char* assetName)
  {
    Tk::Core::StrFixedBuffer<2048> MeshNameStr;
    MeshNameStr.Clear();
    GetCookedDataFileName(assetName, MeshNameStr.EndOfStrPtr(),
                          MeshNameStr.LenRemaining());
    MeshNameStr.UpdateLen();
...
void AssetManager::LoadAllAssets()
{
  ...
  if (m_numMeshAssets > 0)
  {
    ...
    uint32
      meshFileAlreadyCooked[TINKER_MAX_MESHES] = {}; // TODO needs to get replaced with a
    ...
    for (uint32 uiAsset = 0; uiAsset < m_numMeshAssets; ++uiAsset)
    {
      ...
      meshFileAlreadyCooked[uiAsset] =
        (uint32)AssetCooker::IsCookedDataFilePresentForAsset(meshFileNames[uiAsset]);
      if (meshFileAlreadyCooked[uiAsset])
      {
        AssetCooker::GetCookedDataFileName(meshFileNames[uiAsset],
                                           AssetLoadPath.EndOfStrPtr(),
                                           AssetLoadPath.LenRemaining());
    ...
    uint8* meshFileDataBuffer = (uint8*)g_AssetFileScratchMemory.Alloc(
      totalMeshFileBytes + m_numMeshAssets,
      CACHE_LINE); // + 1 EOF byte for each obj file -> + m_numMeshAssets

    // Now precalculate the size of the vertex attribute buffers needed to contain the obj
    // data
      ...
      if (multithreadLoading)
      {
        ...
        for (uint32 uiAsset = 0; uiAsset < m_numMeshAssets; ++uiAsset)
        {
          ...
          jobs.m_jobs[uiAsset] = Platform::CreateNewThreadJob(
            [=]()
            {
              Tk::Platform::ReadEntireFile(AssetLoadPath.m_data, currentMeshFileSize,
                                           currentMeshFile);
    ...
    // Parse mesh files
    {
      ...
      const uint32 MAX_VERT_BUFFER_SIZE =
        1024
        * 1024
        * 128; // TODO: reduce this when we don't have to store all the attr buffers in
               // the same linear allocator by doing graphics right here
      ...
      for (uint32 uiAsset = 0; uiAsset < m_numMeshAssets; ++uiAsset)
      {
        ...
        if (!meshFileAlreadyCooked[uiAsset])
        {
          ...
          Tk::Core::Asset::ParseOBJ(
            VertPosAllocator, VertUVAllocator, VertNormalAllocator, VertIndexAllocator,
            ScratchBuffers, currentMeshFile, currentMeshFileSize, &numObjVerts);
          TINKER_ASSERT(numObjVerts); // TODO: log instead
          m_allMeshData[uiAsset].m_numVertices = numObjVerts;
          m_allMeshData[uiAsset].m_vertexBufferData_Pos =
            VertPosAllocator.m_ownedMemPtr + sizeof(v4f) * accumNumVerts;
          m_allMeshData[uiAsset].m_vertexBufferData_UV =
            VertUVAllocator.m_ownedMemPtr + sizeof(v2f) * accumNumVerts;
          m_allMeshData[uiAsset].m_vertexBufferData_Normal =
            VertNormalAllocator.m_ownedMemPtr + sizeof(v4f) * accumNumVerts;
          m_allMeshData[uiAsset].m_vertexBufferData_Index =
            VertIndexAllocator.m_ownedMemPtr + sizeof(uint32) * accumNumVerts;
          // TODO: create the graphics buffers right away and don't bother storing
          // m_allMeshData at all

          {
            ...
            // NOTE: This is exactly how the graphics buffer will be written to. That code
            // should unified with this logic
            ...
            uint8* cookedDataBufferHeader =
              CookedDataAllocator.Alloc(cookedDataBufferSize, CACHE_LINE);
            uint8* cookedDataBufferPos = cookedDataBufferHeader + sizeof(header);
            uint8* cookedDataBufferUV =
              cookedDataBufferPos + sizeof(v4f) * m_allMeshData[uiAsset].m_numVertices;
            uint8* cookedDataBufferNormal =
              cookedDataBufferUV + sizeof(v2f) * m_allMeshData[uiAsset].m_numVertices;
            uint8* cookedDataBufferIndex =
              cookedDataBufferNormal + sizeof(v4f) * m_allMeshData[uiAsset].m_numVertices;
            TINKER_ASSERT(cookedDataBufferSize
                          == (uint32)(cookedDataBufferIndex - cookedDataBufferHeader)
                               + sizeof(uint32) * m_allMeshData[uiAsset].m_numVertices);
            memcpy(cookedDataBufferHeader, &header, sizeof(header));
            memcpy(cookedDataBufferPos, m_allMeshData[uiAsset].m_vertexBufferData_Pos,
                   sizeof(v4f) * header.numVertices);
            memcpy(cookedDataBufferUV, m_allMeshData[uiAsset].m_vertexBufferData_UV,
                   sizeof(v2f) * header.numVertices);
            memcpy(cookedDataBufferNormal,
                   m_allMeshData[uiAsset].m_vertexBufferData_Normal,
                   sizeof(v4f) * header.numVertices);
            memcpy(cookedDataBufferIndex, m_allMeshData[uiAsset].m_vertexBufferData_Index,
                   sizeof(uint32) * header.numVertices);

            AssetCooker::GetCookedDataFileName(meshFileNames[uiAsset],
                                               AssetLoadPath.EndOfStrPtr(),
                                               AssetLoadPath.LenRemaining());
            AssetLoadPath.UpdateLen();
            Tk::Platform::WriteEntireFile(AssetLoadPath.m_data, cookedDataBufferSize,
                                          cookedDataBufferHeader);
          }
        ...
        else
        {
          ...
          // TODO: this is only to store it until we create the graphics resource. This
          // should get refactored and not happen
          uint8* cookedDataPtr =
            CookedDataAllocator.Alloc(currentMeshFileSize, CACHE_LINE);
          ...
          m_allMeshData[uiAsset].m_vertexBufferData_UV =
            m_allMeshData[uiAsset].m_vertexBufferData_Pos
            + sizeof(v4f)
                * data.header.numVertices; // offset from previous buffer end location
          m_allMeshData[uiAsset].m_vertexBufferData_Normal =
            m_allMeshData[uiAsset].m_vertexBufferData_UV
            + sizeof(v2f) * data.header.numVertices;
          m_allMeshData[uiAsset].m_vertexBufferData_Index =
            m_allMeshData[uiAsset].m_vertexBufferData_Normal
            + sizeof(v4f) * data.header.numVertices;
        }
      }
    }
  }
  ...
  if (m_numTextureAssets > 0)
  {
    ...
    uint8* textureFileDataBuffer =
      (uint8*)g_AssetFileScratchMemory.Alloc(totalTextureFileBytes, CACHE_LINE);
    ...
    if (multithreadTextureLoading)
    {
      ...
      for (uint32 uiAsset = 0; uiAsset < m_numTextureAssets; ++uiAsset)
      {
        ...
        jobs.m_jobs[uiAsset] = Platform::CreateNewThreadJob(
          [=]()
          {
            ReadEntireFile(textureFilePaths[uiAsset], currentTextureFileSize,
                           currentTextureFile);
          });
    ...
    for (uint32 uiAsset = 0; uiAsset < m_numTextureAssets; ++uiAsset)
    {
      ...
      if (strncmp(fileExt, "bmp", 3) == 0)
      {
        ...
        m_allTextureMetadata[uiAsset].m_bitsPerPixel =
          info.bitsPerPixel; // store the original bpp in the texture metadata
      }
      else
      {
        TINKER_ASSERT(0);
    ...
    for (uint32 uiAsset = 0; uiAsset < m_numTextureAssets; ++uiAsset)
    {
      ...
      uint8* textureBuffer =
        m_textureBufferAllocator.Alloc(actualTextureSizes[uiAsset], 1);

      if (strncmp(fileExt, "bmp", 3) == 0)
      {
        uint8* textureBytesStart = currentTextureFile
                                   + sizeof(Tk::Core::Asset::BMPHeader)
                                   + sizeof(Tk::Core::Asset::BMPInfo);

        // Copy the bmp bytes in - no parsing for bmp
        // If the bits per pixel is 24, copy pixel by pixel and write a 0xFF byte for each
        // alpha byte;
        uint32 numPixels =
          m_allTextureMetadata[uiAsset].m_dims.x * m_allTextureMetadata[uiAsset].m_dims.y;
...
void AssetManager::InitAssetGraphicsResources(
  Tk::Graphics::GraphicsCommandStream* graphicsCommandStream)
{
  ...
  for (uint32 uiAsset = 0; uiAsset < m_numMeshAssets; ++uiAsset)
  {
    ...
    Graphics::ResourceHandle stagingBufferHandle_Pos, stagingBufferHandle_UV,
      stagingBufferHandle_Norm, stagingBufferHandle_Idx;
    Tk::Graphics::MemoryMappedBufferPtr stagingBufferMemPtr_Pos, stagingBufferMemPtr_UV,
      stagingBufferMemPtr_Norm, stagingBufferMemPtr_Idx;
    ...
    m_allStaticMeshGraphicsHandles[uiAsset].m_numIndices =
      m_allMeshData[uiAsset].m_numVertices;
    ...
    stagingBufferMemPtr_Pos.MemcpyInto(positionBuffer, numPositionBytes);
    stagingBufferMemPtr_UV.MemcpyInto(uvBuffer, numUVBytes);
    stagingBufferMemPtr_Norm.MemcpyInto(normalBuffer, numNormalBytes);
    stagingBufferMemPtr_Idx.MemcpyInto(indexBuffer, numIndexBytes);

    // Create, submit, and execute the buffer copy commands
    {
      graphicsCommandStream->CmdCommandBufferBegin(
        commandBuffer, "Asset manager create meshes cmd buffer");

      const uint32 numVerts = m_allMeshData[uiAsset].m_numVertices;
      // Position buffer copy
      graphicsCommandStream->CmdCopy(
        stagingBufferHandle_Pos,
        m_allStaticMeshGraphicsHandles[uiAsset].m_positionBuffer.gpuBufferHandle,
        numVerts * sizeof(v4f), "Update Asset Vtx Pos Buf");

      // UV buffer copy
      graphicsCommandStream->CmdCopy(
        stagingBufferHandle_UV,
        m_allStaticMeshGraphicsHandles[uiAsset].m_uvBuffer.gpuBufferHandle,
        numVerts * sizeof(v2f), "Update Asset Vtx UV Buf");

      // Normal buffer copy
      graphicsCommandStream->CmdCopy(
        stagingBufferHandle_Norm,
        m_allStaticMeshGraphicsHandles[uiAsset].m_normalBuffer.gpuBufferHandle,
        numVerts * sizeof(v4f), "Update Asset Vtx Normal Buf");

      // Index buffer copy
      graphicsCommandStream->CmdCopy(
        stagingBufferHandle_Idx,
        m_allStaticMeshGraphicsHandles[uiAsset].m_indexBuffer.gpuBufferHandle,
        numVerts * sizeof(uint32), "Update Asset Vtx Idx Buf");
  ...
  for (uint32 uiAsset = 0; uiAsset < m_numTextureAssets; ++uiAsset)
  {
    ...
    desc.imageUsageFlags =
      Tk::Graphics::ImageUsageFlags::Sampled | Tk::Graphics::ImageUsageFlags::TransferDst;
    ...
    uint32 textureSizeInBytes = m_allTextureMetadata[uiAsset].m_dims.x
                                * m_allTextureMetadata[uiAsset].m_dims.y
                                * 4; // 4 bytes per pixel since RGBA8
    desc.dims = v3ui(textureSizeInBytes, 0, 0);
    desc.resourceType =
      Graphics::ResourceType::eBuffer1D; // staging buffer is just a 1D buffer
    desc.bufferUsage = Graphics::BufferUsage::eStaging;
    desc.debugLabel = "Asset Texture Staging Buffer";
    imageStagingBufferHandles[uiAsset] = Graphics::CreateResource(desc);
    Graphics::MemoryMappedBufferPtr stagingBuffer =
      Graphics::MapResource(imageStagingBufferHandles[uiAsset]);

    // Copy texture data into the staging buffer
    uint8* currentTextureFile =
      m_textureBufferAllocator.m_ownedMemPtr + accumTextureOffset;
    accumTextureOffset += textureSizeInBytes;
    stagingBuffer.MemcpyInto(currentTextureFile, textureSizeInBytes);
  }

  graphicsCommandStream->CmdCommandBufferBegin(commandBuffer,
                                               "Asset manager create textures cmdbuffer");
  ...
  for (uint32 uiAsset = 0; uiAsset < m_numTextureAssets; ++uiAsset)
  {
    ...
    graphicsCommandStream->CmdLayoutTransition(
      m_allTextureGraphicsHandles[uiAsset], Graphics::ImageLayout::eUndefined,
      Graphics::ImageLayout::eTransferDst,
      "Transition image layout to transfer dst optimal");

    graphicsCommandStream->CmdCopy(imageStagingBufferHandles[uiAsset],
                                   m_allTextureGraphicsHandles[uiAsset],
                                   textureSizeInBytes, "Update asset texture data");

    graphicsCommandStream->CmdLayoutTransition(
      m_allTextureGraphicsHandles[uiAsset], Graphics::ImageLayout::eTransferDst,
