...
namespace ForwardRenderPass
{
  RENDER_PASS_EXEC_FUNC(Execute)
  {
    graphicsCommandStream->CmdLayoutTransition(
      ...
      "Transition main view color to transfer_dst");
    graphicsCommandStream->CmdClear(renderPass->colorRTs[0],
                                    v4f(0.1f, 0.25f, 0.25f, 0.0f),
                                    "Clear main view color buffer");
    graphicsCommandStream->CmdLayoutTransition(
      renderPass->colorRTs[0], Tk::Graphics::ImageLayout::eTransferDst,
    ...
    DrawAnimatedPoly(&gameGraphicsData.m_animatedPolygon,
                     Tk::Graphics::SHADER_ID_ANIMATEDPOLY_MainView,
                     Tk::Graphics::BlendState::eAlphaBlend,
                     Tk::Graphics::DepthState::eTestOnWriteOn_CCW, graphicsCommandStream);
